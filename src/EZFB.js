...
(function() {
  ...
  EZFB.appInit = function( $appIdOrOptions /*, $cookie, $logging, $status, $xfbml, $channelUrl, $authResponse, $frictionlessRequests, $hideFlashCallback  */ ) {
  ...
  EZFB.on = EZFB.bind = function( $eventName, $listener ) {
    return event.on( $eventName, $listener );
  };
  EZFB.off = EZFB.unbind = function( $eventName, $listener ) {
    return event.off( $eventName, $listener );
  };
  EZFB.init = function( $listener ) {
    return event.init( $listener );
  };
  EZFB.removeInit = function( $listener ) {
    return event.removeInit( $listener );
  };
  EZFB.like = function( $listener ) {
    return event.like( $listener );
  };
  EZFB.removeLike = function( $listener ) {
    return event.removeLike( $listener );
  };
  EZFB.unlike = function( $listener ) {
    return event.unlike( $listener );
  };
  EZFB.removeUnlike = function( $listener ) {
    return event.removeUnlike( $listener );
  };
  EZFB.loginStatus = function( $listener ) {
    return event.loginStatus( $listener );
  };
  EZFB.removeLoginStatus = function( $listener ) {
    return event.removeLoginStatus( $listener );
  };

  EZFB.update = function() {
    FB.XFBML.parse();
    return this;
  };
  EZFB.canvasAutosize = function() {
    FB.Canvas.setSize();
    FB.Canvas.setAutoGrow();
    return this;
  };
  EZFB.callLoginStatus = function() {
    return api.loginStatus();
  };
  EZFB.getLoginStatus = function() {
    return _loginStatus;
  };
  EZFB.getUserId = function() {
    return _userId;
  };
  EZFB.getAccessToken = function() {
    return _accessToken;
  };
  EZFB.login = function( $scope, $listenerOrRedirectUrl ) {
    return api.login( $scope, $listenerOrRedirectUrl );
  };
  EZFB.logout = function( $listener ) {
  ...
  var _createFBML = function( $event ) {
  ...
  var _applyListeners = function( $eventName ) {
  ...
  var _addListener = function( $eventName, $handler ) {
    if( _eventList[ $eventName ] === undefined ) {
  ...
  var _removeListener = function( $eventName, $handler ) {
    if( _eventList[ $eventName ] === undefined ) {
      return false;
    }
  ...
  var _getLoginStatusHandler = function( $response ) {
   ...
   var _meHandler = function( $response ) {
    EZFB.user( $response );
  };

  if (typeof define === 'function' && define.amd) {
    define('EZFB', [], function() {
      return EZFB;
    });
  }
